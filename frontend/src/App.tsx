import { BarSeries, CandlestickData, CandlestickSeries, ColorType, createChart, IChartApi, LineSeries, OhlcData, Time, LineData } from "lightweight-charts";
import { useEffect, useRef, useState } from "react"
import { HmaStrategy, CandleData, StrategySignal } from "./HmaStrategy"
...
function App() {
    ...
    const hmaStrategyRef = useRef<HmaStrategy>(new HmaStrategy(1000, 2.5));
        ...
        const loadInitialData = async () => {
            try {
                ...
                if (candleSeriesRef.current) {
                    ...
                    // Process historical data with HMA strategy
                    const strategy = hmaStrategyRef.current;
                    const strategySignals = strategy.processHistoricalData(formattedData);
                    setSignals(strategySignals);
                    
                    // Add trend line data
                    const trendLineData = strategy.getTrendLineData();
                    const lineData: LineData[] = trendLineData.map(point => ({
                        time: point.time,
                        value: point.value
                    }));
                    
                    if (trendLineSeriesRef.current && lineData.length > 0) {
                        trendLineSeriesRef.current.setData(lineData);
                    }
        ...
        ws.onmessage = (event) => {
            try {
                ...
                if (message.channel === 'candle' && message.data) {
                    ...
                    // Update HMA strategy with new candle
                    const strategy = hmaStrategyRef.current;
                    const signal = strategy.updateCandle(newCandle);
                    ...
                    // Update trend line
                    const trendLineData = strategy.getTrendLineData();
                    if (trendLineData.length > 0 && trendLineSeriesRef.current) {
